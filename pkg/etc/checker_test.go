...
func TestCheck(t *testing.T) {
	...
	t.Run("Should return error when tunnel cache dir is blank", func(t *testing.T) {
		tempDir := t.TempDir()
		reportsDir := path.Join(tempDir, "reports")

		err := Check(Config{Tunnel: Tunnel{
			CacheDir:   "",
			ReportsDir: reportsDir,
		}})

		assert.EqualError(t, err, "tunnel cache dir must not be blank")
	})

	t.Run("Should return error when tunnel reports dir is blank", func(t *testing.T) {
		tempDir := t.TempDir()
		...
		assert.EqualError(t, err, "tunnel reports dir must not be blank")
	})

	t.Run("Should create tunnel directories", func(t *testing.T) {
		tempDir := t.TempDir()
	...
	t.Run("Should skip creating tunnel directories", func(t *testing.T) {
		tempDir := t.TempDir()
