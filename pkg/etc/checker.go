...
func Check(config Config) error {
	...
	if config.Tunnel.CacheDir == "" {
		return errors.New("tunnel cache dir must not be blank")
	}

	if config.Tunnel.ReportsDir == "" {
		return errors.New("tunnel reports dir must not be blank")
	}

	if err := ensureDirExists(config.Tunnel.CacheDir, "tunnel cache dir"); err != nil {
		return err
	}

	if err := ensureDirExists(config.Tunnel.ReportsDir, "tunnel reports dir"); err != nil {
		return err
	}
