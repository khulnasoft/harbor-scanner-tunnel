...
import (
	...
	"github.com/khulnasoft/harbor-scanner-tunnel/test/component/docker"
	"github.com/khulnasoft/harbor-scanner-tunnel/test/component/scanner"
...
const (
	adapterImage = "khulnasoft/harbor-scanner-tunnel:dev"
...
func TestComponent(t *testing.T) {
	...
	for _, cc := range testCases {
		...
		t.Run(fmt.Sprintf("Should scan %s", imageRef), func(t *testing.T) {
			...
			assert.Equal(t, tunnelScanner, report.Scanner)
			// TODO Adding asserts on CVEs is tricky as we do not have any control over upstream vulnerabilities database used by Tunnel.
